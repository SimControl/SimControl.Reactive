    ...
    public class StateMachineTests: TestFrame
    {
        ...
        public void DoActivity_Cancel()
        {
            ...
            using (sm = new StateMachine(entry: () => Action(-1), exit: () => Action(-1)))
            {
                ...
                Task.Delay(50).Wait(); //TODO why?
                cancellationTokenSource.Cancel();
                Assert.AreEqual(1, TakeAssertTimeout(stateChanged));
                Assert.IsTrue(sm.IsActive("."));
                Assert.IsTrue(sm.IsActive(".SimpleState2"));
        ...
        [Test]
        public void DoActivity_Complex_42() //TODO fix unstable
        {
            //TODO add Action(x)
            int iterations = 3;
        ...
        public void ExceptionTrigger__should__be_queued__when__more_than_exceptions_are_thrown_within_one_transition()
        {
            using (sm = new StateMachine(entry: () => Action(-1), exit: () => Action(-1)))
            {
                ...
                Assert.That(sm.IsActive(".InitialState"), Is.False);
                Assert.That(sm.IsActive(".CompositeState"), Is.False);
                Assert.That(sm.IsActive(".CompositeState.SimpleState1"), Is.False);
                Assert.That(sm.IsActive(".CompositeState.SimpleState2"), Is.False);
                Assert.That(sm.IsActive(".SimpleState3"), Is.False);
        ...
        public void TestMethod02()
        {
            using (sm = new StateMachine(entry: () => Action(-1), exit: () => Action(-1)))
            {
                ...
                Assert.That(sm.IsActive(".InitialState"), Is.False);
                Assert.That(sm.IsActive(".CompositeState.InitialState"), Is.False);
        ...
        public void TestMethod04()
        {
            using (sm = new StateMachine(entry: () => Action(-1), exit: () => Action(-1)))
            {
                ...
                Assert.That(sm.IsActive(".InitialState"), Is.False);
                Assert.That(sm.IsActive(".OrthogonalState.CompositeState1.InitialState"), Is.False);
                Assert.That(sm.IsActive(".OrthogonalState.CompositeState2.InitialState"), Is.False);
        ...
        public void TestMethod07()
        {
            using (sm = new StateMachine(entry: () => Action(-1), exit: () => Action(-1)))
            {
                ...
                Assert.That(sm.IsActive(".InitialState"), Is.False);
                Assert.That(sm.IsActive(".SimpleState1"), Is.False);
        ...
        public void TestMethod08()
        {
            using (sm = new StateMachine(entry: () => Action(-1), exit: () => Action(-1)))
            {
                ...
                Assert.That(sm.IsActive(".InitialState"), Is.False);
                Assert.That(sm.IsActive(".SimpleState1"), Is.False);
                Assert.That(sm.IsActive(".SimpleState2"), Is.False);
        ...
        public void TestMethod15()
        {
            using (sm = new StateMachine(entry: () => Action(-1), exit: () => Action(-1)))
            {
                ...
                Assert.That(sm.IsActive(".CompositeState"), Is.False);
                Assert.That(sm.IsActive(".CompositeState.InitialState"), Is.False);
                Assert.That(sm.IsActive(".CompositeState.SimpleState"), Is.False);
        ...
        private void Add(Channel<int> channel, int i) => Assert.That(channel.Writer.TryWrite(i), Is.True);

        private void Call(int i) => sm.TriggerCallEvent(new CallTrigger<int>(Call), i);

        private Channel<int> CreateChannel() => Channel.CreateUnbounded<int>(
                                                                                                                                                                                                                                                                                                                                            new UnboundedChannelOptions { SingleReader = true, SingleWriter = true });

        private int TakeAssertTimeout(Channel<int> channel) => channel.Reader.ReadAsync().AsTask().AssertTimeoutAsync().Result;
